...
def get_excel_from_google_drive():
    ...
    try:
        ...
        app.logger.error(f"Erro ao baixar ou processar a planilha: {e}")
        return pd.DataFrame()
...
class PDF(FPDF):
    ...
    def header(self):
        ...
        headers = [
            ...
            "GUIA DE MOVIMENTAÇÃO DE BEM MÓVEL PERMANENTE ENTRE AS SEÇÕES DO GAPLS"
        ]
        for header in headers:
            self.cell(0, 6, header, ln=True, align="C")
        self.ln(10)
    ...
    def add_table(self, data):
        ...
        for _, row in data.iterrows():
            ...
            line_count = max(1, self.get_string_width(text) // col_widths[1])
            ...
            valor_atualizado = (
                ...
                if not pd.isnull(row['VL. ATUALIZ.']) else "N/A"
            )
...
Informo à Senhora Chefe do GAP-LS que os bens especificados estão inservíveis para uso neste setor...
...
"""
...
def guia_bens():
    ...
    if request.method == "POST":
        ...
        if not all([bmp_numbers, secao_origem, secao_destino, chefia_origem, chefia_destino]):
            return render_template(
                "guia_bens.html", secoes_origem=secoes_origem, 
                secoes_destino=secoes_destino, error="Preencha todos os campos!"
            )
        ...
        if dados_bmps.empty:
            return render_template(
                "guia_bens.html", secoes_origem=secoes_origem, 
                secoes_destino=secoes_destino, error="Nenhum BMP encontrado."
            )
